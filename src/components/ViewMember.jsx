...
const ViewMember = () => {
  ...
  const renderUserList = (role) => {
    return users
      ...
      .map((user) => (
        <div key={user.id} className="flex items-center p-1 hover:bg-blue-100 ml-2">
          <Image src={user.image} alt="User" width={35} height={35} className="rounded-full" />
          <div className="ml-2">
            <div className=" text-gray-500">{user.name}</div>
          </div>
          <button className="ml-auto" onClick={() => handleDeleteUser(user.id)}>
  ...
  return (
    <div className="text-sm">
      <main className="flex flex-1 w-[33rem]">
        <div className="bg-white overflow-hidden border rounded-xl shadow-md sm:rounded-xl ml-4 p-2 border-blue-300 ">
